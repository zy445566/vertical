...
class Method {
    ...
    static getToken(power_key,is_write)
    {
        let secret_key;
        is_write?secret_key=config.writeKey:secret_key=config.readKey;
        if (secret_key==''){secret_key='noKey';}
        if (power_key==''){power_key='noKey';}
        return new Promise((reslove, reject) => {
            if (secret_key===power_key)
            {
                reslove(jwt.sign({ is_write: is_write }, secret_key));
            } else {
                reject('readKey or WriteKey is error!');
            }
        });
    }

    static jwtVerifyWrite(token)
    {
        ...
        try {
            if(jwt.verify(token, config.writeKey).is_write)
            {
                return true;
            } else {
    ...
    static jwtVerifyRead(token)
    {
        ...
        try {
            return jwt.verify(token, config.readKey).is_write;
        } catch(err) {
            return Method.jwtVerifyWrite(token);
        }
    }

    static getRow(row_key, column_key, timestamp, table,token) {
        return new Promise((reslove, reject) => {
            Method.jwtVerifyRead(token);
    ...
    static getColumn(row_key, column_key, table, limit, reverse, fillCache,token) {
        return new Promise((reslove, reject) => {
            Method.jwtVerifyRead(token);
            let rowList = {};
    ...
    static updateRow(row_key, column_key, row_value, timestamp, table,token) {
        Method.jwtVerifyWrite(token);
        return this.putRow(row_key, column_key, row_value, timestamp, table,token);
    }

    static insertRow(row_key, column_key, row_value, timestamp, table,token) {
        Method.jwtVerifyWrite(token);
        if (timestamp == null) {
    ...
    static putRow(row_key, column_key, row_value, timestamp, table,token) {
        Method.jwtVerifyWrite(token);
        return new Promise((reslove, reject) => {
    ...
    static delRow(row_key, column_key, timestamp, table,token) {
        Method.jwtVerifyWrite(token);
        return new Promise((reslove, reject) => {
            let dataKey = Common.getDataKey(row_key, column_key, timestamp);
            let db = Method.useTable(table);
    ...
    static delColumn(row_key, column_key, table, limit, reverse, fillCache,token) {
        Method.jwtVerifyWrite(token);
        return new Promise((reslove, reject) => {
            let start = Common.getDataKey(row_key, column_key);
            let end = Common.getDataKey(row_key, column_key, 'G');
    ...
    static updateColum(row_key, column_key, row_value, table, limit, reverse, fillCache,token) {
        Method.jwtVerifyWrite(token);
        return new Promise((reslove, reject) => {
            let start = Common.getDataKey(row_key, column_key);
            let end = Common.getDataKey(row_key, column_key, 'G');
    ...
    static insertColum(row_key, column_key, row_value_list_str, table,token) {
        Method.jwtVerifyWrite(token);
    ...
    static InitServer() {
        if (config.readKey==''){config.readKey='noKey';}
        if (config.writeKey==''){config.writeKey='noKey';}
        //生成服务器唯一标识
    ...
    static isSync(serverSign, timestamp,token) {
    ...
    static writeSyncData(syncWriteDataStr,token) {
    ...
    static getClient(server) {
        ...
        isSyncNow[server] = false;
        ClientList[server] = new Client(server, config.serverPort, config.writeKey, true);
        return ClientList[server];
    }
        ...
        if (promiseFunc == null) {
            ...
            promiseFunc
                .then((res) => {
                    ...
                    result(new VerticalTypes.VerticalError({ message: err.toString() }));
                });
        }
    }
}
